...
logger = logging.getLogger("earthpy.project")
...
class Project:
    ...
    def _warn_deprecated_verbose(self, verbose):
        """Emit a single deprecation warning for verbose usage."""
    ...
    def _load_config_files(self):
        ...
        for ext in supported_exts:
            ...
            if local_file.exists():
                logger.info("Loading local configuration from %s", local_file)
                local_configs.update(self._read_config_file(local_file))

            # Load global config if it exists
            if global_file.exists():
                logger.info("Loading global configuration from %s", global_file)
        ...
        logger.info("Final configuration loaded: %s", combined_config)
    ...
    def _get_config_parameter(self, param_name):
        ...
        for key, value in os.environ.items():
            if key.lower() == f"EARTHPY_{param_name}".lower():
                logger.debug("Found '%s' in environment variables.", param_name)
                return value

        # Configuration file check
        if param_name in self.config:
            logger.debug("Found '%s' in configuration files.", param_name)
