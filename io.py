...
logger = logging.getLogger("earthpy.io")
...
def path_to_example(fname):
    ...
    # Fallback for alternate layouts while preserving the public API.
    fallback = pathlib.Path(__file__).resolve().parent.parent / "example-data" / fname
    if fallback.exists():
        return fallback
...
class Data(object):
    ...
    def __init__(self, project=None, verbose=False):
        ...
        self._verbose_deprecation_warning_shown = False
        self._warn_deprecated_verbose(verbose)
    ...
    def _warn_deprecated_verbose(self, verbose):
        """Log a single deprecation warning when verbose is used."""
        if verbose and not self._verbose_deprecation_warning_shown:
            logger.warning(
                "The 'verbose' parameter is deprecated; configure the 'earthpy' "
                "logger to control output."
            )
            self._verbose_deprecation_warning_shown = True
    ...
    def get_data(self, 
        ...
        if (key is None) and (url is None) and (title is None):
            if self.project:
                ...
                logger.info("No key, url, or title provided. Available datasets:")
                self.list_datasets()
                return
    ...
    def _get_figshare_download_urls(self, article_id):
        ...
        if not files:
            logger.warning("No files found for article %s.", article_id)
            return {}
    ...
    def _download_and_extract(self, path, r, kind, verbose):
        ...
        logger.info("Extracted output to %s", path)
    ...
    def _zip_dir(self, dir_path, zip_home, zipf, verbose=False):
        ...
        for subfile in dir_path.rglob("*"):
            if (subfile.name in DVCIGNORE):
                logger.debug("Skipping %s as it is in DVCIGNORE.", subfile)
                continue
            if subfile.is_file():
                dest_path = subfile.relative_to(zip_home)
                logger.debug("Zipping %s to %s", subfile, dest_path)
    ...
    def prepare_for_upload(self, verbose=False):
        ...
        for path in files_and_dirs:
            if path.is_file():
                ...
                logger.debug("Copying file: %s to %s", path, dest_file)
                dest_file.write_bytes(path.read_bytes())
            if path.is_dir():
                zipfile_name = output_path / f"{path.name}.zip"
                with zipfile.ZipFile(zipfile_name, "w") as zipf:
                    logger.debug("Zipping directory: %s to %s", path, zipfile_name)
                    self._zip_dir(path, path, zipf)
        logger.info("Prepared files for upload in %s", output_path)
        return output_path