...
class AppeearsDownloader(APIDownloader):
    ...
    def appeears_request(
        ...
        **parameters : dict, optional
        ...
        if req_json:
            logging.debug(
                "Submitting task with JSON\n{}".format(json.dumps(req_json))
            )
    ...
    def login(self, interactive=True, override=False):
        ...
        username, password = Authenticator(
            service=self.login_service, env_prefix="EARTHDATA"
        ).get_credentials(interactive=interactive, override=override)

        # Set up authentication and submit login request
        login_resp = requests.post(
            self.base_url + "login", auth=(username, password)
        )
        login_resp.raise_for_status()

        self._auth_header = "{token_type} {token}".format(**login_resp.json())
    ...
    def submit_task_request(self):
        ...
        task = {
            ...
            "params": {
                "dates": [
                    {"startDate": self._start_date, "endDate": self._end_date}
                ],
                "layers": [{"product": self._product, "layer": self._layer}],
        ...
        if self._recurring:
            if self._year_range is None:
                raise ValueError("Must supply year range for recurring dates")
    ...
    def download_files(self, cache=True):
        ...
        for file_info in files:
            ...
            if filepath.exists() and cache:
                logging.info(
                    "File at {} alreading exists. Skipping...".format(filepath)
                )
            else:
