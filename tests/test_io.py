...
def test_verbose_arg_works(data_instance, caplog):
    """Test that verbose behavior emits logger output rather than raw prints."""
    with caplog.at_level("INFO", logger="earthpy"):
        data_instance.get_data("little-text-file", verbose=True)
    assert any("Downloading from" in record.getMessage() for record in caplog.records)


@skip_on_ci
@pytest.mark.vcr()
def test_url_download_with_quotes(data_instance):
    """Check that zip downloads with quoted names still extract cleanly.

    This uses the tiny Figshare archive already defined in DATA_URLS. It exercises
    the same zip extraction path without depending on an unstable external API.
    """
    path = data_instance.get_data("little-zip-file")
    files = os.listdir(path)
    assert len(files) > 0 and os.path.isdir(path)
